...
public class FindBridiByIdServiceStub implements BridiTestData {
	public static FindBridiByIdService stub() {
		...
		when(mock.apply(ParserConstants.QUERY_BRIDI_ID)).thenReturn(Optional.of(ANY));
		when(mock.apply(SUMTI_IS_A_THING_ID)).thenReturn(Optional.of(SUMTI_IS_A_THING));
		when(mock.apply(TAUTOLOGY_ID)).thenReturn(Optional.of(TAUTOLOGY));
		when(mock.apply(THING_IS_A_SUMTI_ID)).thenReturn(Optional.of(THING_IS_A_SUMTI));
