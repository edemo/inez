...
public class GetBridiIdBySelbriAndSumtiIdsServiceStub implements BridiFieldTestData {
	public static GetBridiIdBySelbriAndSumtiIdsService stub() {
		...
		when(mock.apply(IS_A_ID, THING_ID, 2))
				.thenAnswer((args) -> Stream.of(
						TAUTOLOGY_ID, SUMTI_IS_A_THING_IS_A_THING_ID, SUMTI_IS_A_THING_ID, TAUTOLOGY_IS_A_THING_ID));
		when(mock.apply(IS_A_ID, IS_A_ID, 0))
				.thenAnswer((args) -> Stream.of(SUMTI_IS_A_THING_ID, SUMTI_IS_A_THING_IS_A_THING_ID));
		when(mock.apply(IS_A_ID, SUMTI_IS_A_THING_IS_A_THING_ID, 1))
				.thenAnswer((args) -> Stream.of(SUMTI_IS_A_THING_IS_A_THING_ID));
		when(mock.apply(IS_A_ID, SUMTI_IS_A_THING_ID, 1)).thenAnswer((args) -> Stream.of(SUMTI_IS_A_THING_ID));
